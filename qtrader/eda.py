...
from numpy import zeros, log, round, array, unique
import matplotlib.pyplot as plt
import matplotlib.dates as dates
from matplotlib import ticker as mticker
from pandas import DataFrame, Series, concat, to_datetime, read_csv
from seaborn import color_palette, barplot
...
def make_df(d_data):
    ...
    '''
    df_aux = DataFrame(d_data)
    df_filter = Series([x.day for x in df_aux.index])
    df_aux2 = DataFrame(zeros(df_aux.shape))
        ...
        if row['Type'] in ['BID', 'ASK']:
            ...
            if f_current_time > f_next_time:
                ...
                if f_mid:
                    ...
                    f_logrtn = log((f_curent_mid/f_mid))
...
def cluster_results(reduced_data, preds, centers):
    ...
    predictions = DataFrame(preds, columns=['Cluster'])
    plot_data = concat([predictions, reduced_data], axis=1)

    # Generate the cluster plot
    fig, ax = plt.subplots(figsize=(10, 6))

    # Color map
    # cmap = color_palette('cubehelix', 12)
    cmap = color_palette('Set2', 12)
    ...
    with open(s_fname) as fr:
        ...
        for idx, row in enumerate(fr):
            ...
            if '{}.update'.format(s_agent) in s_aux:
                ...
                s_x = s_date_all[:-3]
                s_date = s_x
                ts_date_all = to_datetime(s_date_all)
                ts_date = to_datetime(s_date + ':00')
...
def plot_train_test_sim(d_rtn):
    ...
    for ax1, idx in zip(na_ax.ravel(), list(range(df_test.shape[1]))):
...
def plot_cents_changed(archive, archive2):
    ...
    # load data
    df_prices = read_csv(archive.open(l_fnames[6]),
                            index_col=0,
                            parse_dates=['Date'])
    df_prices = df_prices[df_prices.Type == 'TRADE']
    for idx in [16, 26, 36]:
        df_aux = read_csv(archive.open(l_fnames[idx]),
                             index_col=0,
                             parse_dates=['Date'])
        df_aux = df_aux[df_aux.Type == 'TRADE']
        df_prices = concat([df_prices, df_aux], ignore_index=True)
    ...
    df_prices2 = read_csv(archive2.open(l_fnames2[0]),
                             sep='\t',
                             index_col=0,
                             parse_dates=['DATE'],
                             dayfirst=True,
    ...
    # filer data
    na_hour = array([x.hour*60 + x.minute for x in quotes.index])
    quotes = quotes[(na_hour >= (10 * 60 + 30)) & (na_hour <= (16 * 60 + 30))]
    na_days = array([x.day for x in quotes.index])
